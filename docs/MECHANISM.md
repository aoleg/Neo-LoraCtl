...
## Compile mode and Seed Variance (bake-based fast paths)

Both features avoid the online path entirely by cooking strengths into the baked weights.

**Compile** (UI checkbox, default on): when no time curve runs, scheduled LoRAs are routed baked and every freshly added payload's strength is multiplied by its block factor at interception time (`loractl_core.scale_new_baked_patches`), before any weight is touched. The stock load path then merges them once; generation runs at native speed with zero per-step work. Block-config changes are folded into the membership signature in this mode, so they null `current_lora_hash` and re-bake via a stock reload. Memory is identical to plain LoRA use: Forge's own backup/restore lifecycle applies unchanged.

**Seed Variance**: a dropdown-selected helper LoRA (not prompt-loaded) is baked at strength X by adding baked patches to the per-generation unet in `process_before_every_sampling` — NOT `process_batch`: Forge resets `forge_objects` from the post-LoRA snapshot inside `sample()` (`modules/processing.py:1376`), so anything attached earlier is silently discarded (found live: pixel-identical output). The TE half, when enabled, is applied separately in `process_batch`, before the conditioning is encoded. The attach runs once per batch; the hires pass starts from a fresh reset copy and deliberately runs without the SV LoRA. `core.BakedLoraHandle` tracks the payloads by adapter identity. The uuid bump makes Forge's loader bake it on the next `sampling_prepare` — `partially_load` (`backend/patcher/base.py:773`) unpatches (full backup restore) on any uuid mismatch before repatching, which is the load-bearing guarantee for every dict-level transition. At the first step below the composition boundary (normalized sigma, default 0.90), the payload strengths are set to 0 and the affected keys re-baked by `_rebake_keys`: a mirror of `patch_weight_to_device` that sources from `patcher.backup` instead of the live weight (merging onto the live weight would double-apply — the one trap in this design). Keys with no other patches take a pure-restore path (bit-exact, quantization included); shared keys re-merge the other LoRAs' payloads. Keys with no backup (lowvram, applied per forward) need no surgery at all. Per batch: one re-bake at batch start, one de-bake at the boundary. `postprocess` enforces the cleanup invariant unconditionally — restore if the run was interrupted before the boundary, remove the payloads from the dict (weights and dict stay consistent, no uuid churn), and drop backup entries that exist solely because of this LoRA.
